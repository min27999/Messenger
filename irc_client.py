...
class IRCSimpleClient:
    ...
    def __init__(self, username, channel, server="125.240.245.248", port=1234):
...
def running_client(username, channel):
    cmd = ""
    joined = False
    ...
    while(cmd != "/quit"):
        ...
        response_thread = threading.Thread(target=lambda: print_response(client))
        response_thread.daemon = True
        response_thread.start()

# helper function used as thread target
def print_response(client: IRCSimpleClient):
    resp = client.get_response()
    if resp:
        msg = resp.strip().split(":")
        print("< {}> {}".format(msg[1].split("!")[0], msg[2].strip()))
        # return "< {}> {}".format(msg[1].split("!")[0], msg[2].strip())
